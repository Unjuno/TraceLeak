#!/usr/bin/env python3
"""Build an OpenSSL metadata-only model-sequence sample model preflight."""
...
from traceleak.openssl_model_sequence_metadata_sample_model_preflight import (
    ...
    write_openssl_model_sequence_metadata_sample_model_preflight,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a preflight for an OpenSSL metadata-only model-sequence sample."
    )
...
def main() -> int:
    ...
    try:
        ...
        write_openssl_model_sequence_metadata_sample_model_preflight(args.out, preflight)
    except Exception as exc:
        print(f"invalid OpenSSL metadata sample model preflight request: {exc}")
        return 1
    print(
        "wrote OpenSSL metadata sample model preflight: "
        f"{args.out} ({preflight['record_count']} record(s))"
    )
    return 0
