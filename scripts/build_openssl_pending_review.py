#!/usr/bin/env python3
"""Build a pending OpenSSL review template."""
...
from traceleak.openssl_human_review_checklist import OpenSSLHumanReviewChecklistError
...
from traceleak.openssl_review_record_template import (
    ...
    write_review_record_template_json,
    write_review_record_template_markdown,
)
...
def main() -> int:
    args = parse_args()
    try:
        template = build_openssl_review_record_template(
        ...
        return 1
    if args.format == "json":
        write_review_record_template_json(args.output_path, template)
    else:
        write_review_record_template_markdown(args.output_path, template)
    print(f"wrote OpenSSL pending review template: {args.output_path}")
    return 0
