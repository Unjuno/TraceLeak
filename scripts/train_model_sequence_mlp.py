#!/usr/bin/env python3
"""Train a one-hidden-layer local MLP over model sequence token counts.

Usage:
  python scripts/train_model_sequence_mlp.py --in examples/synthetic/model_sequence_baseline_sample.json --out model_result.json
"""
...
from traceleak.model_sequence_nn import ModelSequenceNNError


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a local model-sequence MLP classifier.")
    ...
    parser.add_argument("--experiment-id", default="exp_model_sequence_mlp_local")
    parser.add_argument("--epochs", default=200, type=int, help="Training epochs")
    parser.add_argument("--learning-rate", default=0.2, type=float, help="SGD learning rate")
    parser.add_argument("--l2", default=0.0001, type=float, help="L2 regularization strength")
    parser.add_argument("--hidden-size", default=8, type=int, help="Hidden layer width")
    parser.add_argument("--top-k-attributions", default=8, type=int)
...
def main() -> int:
    args = parse_args()
    try:
        result = train_model_sequence_mlp_result(
    ...
    print(f"wrote model sequence MLP result: {args.output_path}")
    return 0
