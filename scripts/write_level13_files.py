...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Write Level 13 closure files.")
    parser.add_argument("--out-dir", default=Path("reports/local/level13_closure"), type=Path)
...
def main() -> int:
    ...
    try:
        ...
        report = render_level13_closure_report(
            closure_manifest=closure,
            handoff_inventory=inventory,
        )
        write_level13_closure_manifest(args.out_dir / "level13-closure-manifest.json", closure)
        write_level13_handoff_inventory(args.out_dir / "level13-handoff-inventory.json", inventory)
        write_level13_closure_report(args.out_dir / "level13-closure-report.md", report)
    except Exception as exc:
        print(f"invalid level13 request: {exc}")
        return 1
    print(f"wrote level13 closure files: {args.out_dir}")
    return 0
