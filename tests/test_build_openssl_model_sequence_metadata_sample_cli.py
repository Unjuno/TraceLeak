...
from scripts import build_openssl_model_sequence_metadata_sample as cli
...
def make_chain() -> tuple[dict, dict, dict, dict, dict, dict, dict]:
...
def write_chain(tmp_path, chain: tuple[dict, dict, dict, dict, dict, dict, dict]) -> tuple:
    names = [
...
def test_build_openssl_model_sequence_metadata_sample_cli_writes_sample(
    tmp_path,
    monkeypatch,
) -> None:
    paths = write_chain(tmp_path, make_chain())
    out_path = tmp_path / "openssl-model-sequence-metadata-sample.json"
    monkeypatch.setattr(
        "sys.argv",
        [
    ...
    assert payload["artifact_format"] == "traceleak.openssl_model_sequence_metadata_sample.v1"
    assert payload["run_count"] == 4
    assert payload["sample_metadata"]["phase"] == "P22"
    assert payload["sample_metadata"]["metadata_only"] is True
    assert payload["sample_metadata"]["payload_free"] is True


def test_build_openssl_model_sequence_metadata_sample_cli_rejects_bad_record_count(
    tmp_path,
    monkeypatch,
) -> None:
    paths = write_chain(tmp_path, make_chain())
