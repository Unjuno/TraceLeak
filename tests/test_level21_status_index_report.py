from traceleak.level21_status_index import (
    ...
    validate_level21_status_index_report,
    write_level21_status_index_report,
)


def test_status_index_report_sections() -> None:
    index = build_level21_status_index()

    markdown = render_level21_status_index_report(index)

    assert "# Level 21 Status Index Report" in markdown
...
def test_status_index_report_writer(tmp_path) -> None:
    index = build_level21_status_index()
    markdown = render_level21_status_index_report(index)
    path = tmp_path / "level21-status-index-report.md"

    write_level21_status_index_report(path, markdown)

    assert path.read_text(encoding="utf-8").startswith("# Level 21 Status Index Report")
