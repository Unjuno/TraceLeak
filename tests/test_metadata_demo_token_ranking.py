...
def test_metadata_demo_token_ranking_builds_from_demo_outputs() -> None:
    artifacts = build_openssl_metadata_demo_chain(epochs=20)
    ranking = build_metadata_demo_token_ranking(
        demo_manifest=artifacts["demo_manifest"],
    ...
    assert ranking["sample_digest"] == artifacts["demo_manifest"]["sample_digest"]
...
def test_metadata_demo_token_ranking_rejects_non_numeric_score() -> None:
    artifacts = build_openssl_metadata_demo_chain(epochs=20)
    ranking = build_metadata_demo_token_ranking(
        demo_manifest=artifacts["demo_manifest"],
        nn_result=artifacts["nn_result"],
...
def test_metadata_demo_token_ranking_rejects_real_world_claim() -> None:
    artifacts = build_openssl_metadata_demo_chain(epochs=20)
    ranking = build_metadata_demo_token_ranking(
        demo_manifest=artifacts["demo_manifest"],
        nn_result=artifacts["nn_result"],
