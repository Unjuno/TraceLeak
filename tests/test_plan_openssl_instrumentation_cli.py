...
def test_plan_openssl_instrumentation_cli_writes_markdown(tmp_path) -> None:
    source_pin_path = make_source_pin(tmp_path)
    output_path = tmp_path / "patch_plan.md"

    result = subprocess.run(
        [
            sys.executable,
            "scripts/plan_openssl_instrumentation.py",
            "--source-pin",
            str(source_pin_path),
            "--event-map",
            str(EVENT_MAP),
...
def test_plan_openssl_instrumentation_cli_writes_json(tmp_path) -> None:
    source_pin_path = make_source_pin(tmp_path)
    output_path = tmp_path / "patch_plan.json"
    ...
    assert payload["format"] == "traceleak.openssl_patch_plan.v1"
    assert payload["patch_application_allowed"] is False
    assert len(payload["planned_events"]) == 5


def test_plan_openssl_instrumentation_cli_rejects_missing_source_pin(tmp_path) -> None:
    result = subprocess.run(
        [
            sys.executable,
            "scripts/plan_openssl_instrumentation.py",
