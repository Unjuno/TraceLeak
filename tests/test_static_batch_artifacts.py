...
from traceleak.static_batch_artifacts import (
    STATIC_BATCH_INDEX_FORMAT,
    write_static_batch_artifacts,
    write_static_batch_split_artifact,
)
...
def test_write_static_batch_artifacts_writes_index_and_samples(tmp_path) -> None:
    ...
    written_split = write_static_batch_split_artifact(split=split, output_dir=output_dir)
    written_index = json.loads((output_dir / "index.json").read_text(encoding="utf-8"))
    split_payload = json.loads((output_dir / "split.json").read_text(encoding="utf-8"))

    assert index == written_index
    assert written_split == split_payload
    ...
    assert split_payload["metadata"]["eval_modules"] == ["crypto/evp"]
