...
from scripts import validate_openssl_materialization_output_manifest as cli
...
def test_validate_openssl_materialization_output_manifest_cli_accepts_valid_pair(
    tmp_path,
    monkeypatch,
) -> None:
...
def test_validate_openssl_materialization_output_manifest_cli_rejects_mismatch(
    tmp_path,
    monkeypatch,
) -> None:
    contract, manifest = make_contract_and_manifest()
    manifest["trace_contract_digest"] = "sha256:other-trace-contract"
    contract_path = tmp_path / "contract.json"
    manifest_path = tmp_path / "manifest.json"
    contract_path.write_text(json.dumps(contract), encoding="utf-8")
    manifest_path.write_text(json.dumps(manifest), encoding="utf-8")
