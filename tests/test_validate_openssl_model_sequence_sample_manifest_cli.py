...
from scripts import validate_openssl_model_sequence_sample_manifest as cli
...
def make_contract_and_manifest() -> tuple[dict, dict]:
    ...
    return sample_contract, sample_manifest


def test_validate_openssl_model_sequence_sample_manifest_cli_accepts_valid_pair(
    tmp_path,
    monkeypatch,
) -> None:
...
def test_validate_openssl_model_sequence_sample_manifest_cli_rejects_mismatch(
    tmp_path,
    monkeypatch,
) -> None:
    contract, manifest = make_contract_and_manifest()
    manifest["label_fields"] = ["other_label"]
    contract_path = tmp_path / "contract.json"
    manifest_path = tmp_path / "manifest.json"
    contract_path.write_text(json.dumps(contract), encoding="utf-8")
    manifest_path.write_text(json.dumps(manifest), encoding="utf-8")
