...
from scripts import validate_openssl_model_sequence_sample_materialization_output_manifest as cli
...
def test_validate_sample_materialization_output_manifest_cli_accepts_valid_chain(
    tmp_path,
    monkeypatch,
) -> None:
    (
...
def test_validate_sample_materialization_output_manifest_cli_rejects_mismatch(
    tmp_path,
    monkeypatch,
) -> None:
    (
    ...
    output_manifest["sample_digest"] = "sha256:other-sample"
    sample_contract_path = tmp_path / "sample-contract.json"
    sample_manifest_path = tmp_path / "sample-manifest.json"
    approval_record_path = tmp_path / "approval-record.json"
    approval_gate_path = tmp_path / "approval-gate.json"
